# =============================================================================
# 6_Test_all_visual_confusion.py (UPDATED)
# =============================================================================
# اضافه شدن:
#   1) حذف خطوط سبز/آبی از تصاویر خام با median filter
#   2) تولید تصاویر برش‌خورده با border 5 پیکسل
# =============================================================================
...
class CONFIG:
    ...
    # تنظیمات حذف خطوط
    REMOVE_LINES = True  # فعال‌سازی حذف خطوط سبز و آبی
    LINE_NEIGHBORHOOD = 5  # همسایگی اولیه
    FALLBACK_NEIGHBORHOOD = 8  # همسایگی ثانویه
...
# =============================================================================
# LINE REMOVAL UTILITIES
# =============================================================================

def is_color_line_pixel(pixel, target_colors, tolerance=30):
    """
    Check if a pixel is close to green (0,255,0) or blue (0,0,255)
    """
    if len(pixel) < 3:
        return False
    ...
    if abs(r - 0) < tolerance and abs(g - 255) < tolerance and abs(b - 0) < tolerance:
        return True
    
    # Blue: (0, 0, 255) with tolerance
    if abs(r - 0) < tolerance and abs(g - 0) < tolerance and abs(b - 255) < tolerance:
        return True
    
    return False


def get_neighborhood_pixels(img, y, x, radius):
    """
    Get pixel values from neighborhood excluding center
    """
    h, w = img.shape[:2]
    pixels = []
...
def remove_colored_lines_from_image(img_array, neighborhood=5, fallback=8, tolerance=30):
    """
    Remove green and blue lines from image using median filtering
    """
    img = img_array.copy()
    h, w = img.shape[:2]
    
    # Find colored line pixels
    line_pixels = []
    for y in range(h):
        for x in range(w):
            if is_color_line_pixel(img[y, x], None, tolerance):
                line_pixels.append((y, x))
    
    if not line_pixels:
        return img
    
    print(f"[Line Removal] Found {len(line_pixels)} colored pixels to fix")
    
    # Fix each line pixel
    for y, x in line_pixels:
        # Try first neighborhood
        neighbors = get_neighborhood_pixels(img, y, x, neighborhood)
        non_line_neighbors = [p for p in neighbors if not is_color_line_pixel(p, None, tolerance)]
        
        if non_line_neighbors:
            med = median_color(non_line_neighbors)
            if med is not None:
                img[y, x] = med
                continue
        
        # Try fallback neighborhood
        neighbors = get_neighborhood_pixels(img, y, x, fallback)
        non_line_neighbors = [p for p in neighbors if not is_color_line_pixel(p, None, tolerance)]
...
def plot_confusion_matrix_normalized(cm, metrics, title, out_path):
    """
    Plot normalized confusion matrix as percentages
    """
    # Normalize rows
    row_sums = cm.sum(axis=1, keepdims=True)
    cm_norm = cm.astype(np.float32) / (row_sums + 1e-7)
    cm_norm = cm_norm * 100  # Convert to percentage
    ...
    base_path = out_path.parent / (out_path.stem + "_normalized" + out_path.suffix)
    fig.savefig(base_path, dpi=CONFIG.CM_DPI)
    plt.close(fig)
...
# =============================================================================
# CROP PANEL GENERATION (NEW)
# =============================================================================

def bbox_from_mask_with_border(mask_bool, border=5):
    """
    Get bounding box of mask and add border pixels
    """
    h, w = mask_bool.shape
    ...
    x0 = max(0, xs.min() - border)
    x1 = min(w, xs.max() + 1 + border)
    y0 = max(0, ys.min() - border)
    y1 = min(h, ys.max() + 1 + border)
    
    return x0, y0, x1, y1


def make_crop_panel_image(
    raw_rgb,
    gt_mask,
    pred_mask,
    heatmap,
    ...
    """
    Save crop panel image with:
    1. Raw image with crop box
    2. GT mask with crop box
    3. Raw + GT mask (red)
    4. Crop location (G,R) - green GT, red pred
    5. Crop mask (G,B) - green GT, blue pred
    ...
    crop_rgb = raw_rgb[y0:y1, x0:x1].copy()
    crop_gt = gt_mask[y0:y1, x0:x1].copy()
    crop_pred = pred_mask[y0:y1, x0:x1].copy()
    crop_heat = heatmap[y0:y1, x0:x1].copy()
    
    # Panel 1: Raw with crop box
    raw_with_box = raw_rgb.copy()
    raw_with_box = draw_bbox(raw_with_box, bbox, color=(255, 0, 0), thickness=3)
    
    # Panel 2: GT mask with crop box
    gt_vis = np.zeros_like(raw_rgb)
    gt_vis[gt_mask] = np.array([255, 255, 255], dtype=np.uint8)
    gt_vis = draw_bbox(gt_vis, bbox, color=(255, 0, 0), thickness=3)
    
    # Panel 3: Raw + GT mask (red)
    raw_gt_red = overlay_mask(raw_rgb, gt_mask, color=(255, 0, 0), alpha=0.45)
    raw_gt_red = draw_bbox(raw_gt_red, bbox, color=(255, 0, 0), thickness=3)
    
    # Panel 4: Crop location (G,R) - green GT, red pred
    crop_loc = crop_rgb.copy()
    crop_loc = overlay_mask(crop_loc, crop_gt, color=(0, 255, 0), alpha=0.60)
    crop_loc = overlay_mask(crop_loc, crop_pred, color=(255, 0, 0), alpha=0.60)
    
    # Panel 5: Crop mask (G,B) - green GT, blue pred
    crop_mask_vis = crop_rgb.copy()
    crop_mask_vis = overlay_mask(crop_mask_vis, crop_gt, color=(0, 255, 0), alpha=0.60)
    crop_mask_vis = overlay_mask(crop_mask_vis, crop_pred, color=(0, 80, 255), alpha=0.60)
    
    # Panel 6: Intersection
    intersection = np.zeros_like(crop_rgb).astype(np.uint8)
    gt_only = crop_gt & (~crop_pred)
    pred_only = crop_pred & (~crop_gt)
    both = crop_gt & crop_pred
    
    intersection[gt_only] = np.array([0, 255, 0], dtype=np.uint8)       # GT only
    intersection[pred_only] = np.array([255, 0, 0], dtype=np.uint8)     # Pred only
    intersection[both] = np.array([255, 255, 0], dtype=np.uint8)        # overlap
    
    panels = [
        ("1. Raw + box", raw_with_box),
        ("2. GT mask + box", gt_vis),
        ("3. Raw+GT(red)", raw_gt_red),
        ("4. Crop loc (G,R)", crop_loc),
        ("5. Crop mask (G,B)", crop_mask_vis),
    ...
    if metrics is not None:
        txt = f"Dice={metrics['dice']:.4f} | IoU={metrics['iou']:.4f}"
        fig.text(0.5, 0.02, txt, ha="center", fontsize=11)
    ...
    return crop_rgb, crop_gt, crop_pred, crop_heat


def draw_bbox(rgb, bbox, color=(255, 0, 0), thickness=3):
    """
...
def process_dataset(dataset_name, dataset_dir, model, cam_engine):
    ...
    crop_panel_dir = out_root / "crop_panels"  # NEW: crop panel directory

    ensure_dir(out_root)
    ensure_dir(panel_dir)
    ensure_dir(crop_panel_dir)  # NEW
    ...
    for idx, item in enumerate(pairs):
        ...
        try:
            ...
            if CONFIG.REMOVE_LINES:
                raw_rgb = remove_colored_lines_from_image(
                    raw_rgb,
                    neighborhood=CONFIG.LINE_NEIGHBORHOOD,
                    fallback=CONFIG.FALLBACK_NEIGHBORHOOD
                )
            # =====================================================
            ...
            # ==================== CROP PANEL (NEW) ====================
            if n_crop_panels < max_vis:
                # Get bbox with 5 pixel border
                bbox = bbox_from_mask_with_border(gt_mask, border=5)
                
                crop_name = f"{idx:04d}_{item['split']}_{item['stem']}_crop.png"
                out_crop_path = crop_panel_dir / crop_name

                make_crop_panel_image(
                    raw_rgb=raw_rgb,
                    gt_mask=gt_mask,
                    pred_mask=pred_mask,
                    heatmap=cam_full,
    ...
    cm_norm_path = plot_confusion_matrix_normalized(
        cm=cm_total,
        metrics=metrics_total,
        title=f"{dataset_name} Pixel-level Confusion Matrix",
        out_path=cm_png
    ...
    summary = {
        ...
        "num_crop_panel_images": n_crop_panels,  # NEW
        "output_dir": str(out_root),
        "crop_panel_dir": str(crop_panel_dir),  # NEW
        "confusion_matrix": cm_total.tolist(),
        "confusion_matrix_normalized": (cm_total / (cm_total.sum(axis=1, keepdims=True) + 1e-7)).tolist(),  # NEW
        "metrics": metrics_total,
        "line_removal": {
            "enabled": CONFIG.REMOVE_LINES,
            "neighborhood": CONFIG.LINE_NEIGHBORHOOD,
            "fallback": CONFIG.FALLBACK_NEIGHBORHOOD
        }  # NEW
    }
    ...
    print(f"Crop Panels: {n_crop_panels}")  # NEW
    ...
    print(f"Confusion Normalized PNG: {cm_norm_path}")  # NEW
    print("=" * 80 + "\n")
...
def main():
    print("=" * 80)
    print("6_Test_all_visual_confusion.py (UPDATED)")
    print("=" * 80)
    ...
    print(f"Line Removal: {CONFIG.REMOVE_LINES}")
    print("=" * 80)
